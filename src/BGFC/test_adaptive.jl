...
atc = AtC(Ra, bw, Lmsh, 15, calc; meshpath=meshpath);
...
obj_g = x -> nsoli_condition_gradient(atc, x, P)
@time x, it_hist, ierr, x_hist = nsoli(JuAtC.get_x(atc), obj_g; atol=1e-3, rtol=1e-3, debug = 1, lmaxit=30, maxit=30)
@show size(it_hist, 1)
